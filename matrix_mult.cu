...
{	
	...
	for(int j=0; j < H; j++) 
	{
		for(int i=0; i < X; i++) { 
				...
				printf("(%4.4g,%4.4g,%4.4g)", mat[(i*3)+0 + j*X],mat[(i*3)+1 + j*X],mat[(i*3)+2 + j*X]);
...
matmul_parallel(float input[], float weight[], float out[], float d_out_debug[])
{
	...
	d_out_debug[intWeightX*3] = gridDim.x;
	d_out_debug[intWeightX*3+1] = blockDim.x;
	d_out_debug[intWeightX*3+2] = threadIdx.x;
	//__syncthreads();
 ...
 */
	...
	printf("blocks %d\n",N/K);	
	print_matrix(weight);
	matmul_parallel<<<blocks,threads>>>(d_input,d_weight, d_out,d_out_debug);
