...
exp_CPU(float* input, int numRows, int numColumns, float* out){
	...
	for(int i = 0; i< numRows; i++){
		for(int j = 0; j< numColumns; j++){
			...
			temp +=out[i*numColumns + j];
		}
	}
	for(int i = 0; i< numRows; i++){
		for(int j = 0; j< numColumns; j++){
			out[i*numColumns + j] = out[i*numColumns + j]/temp;
		}
	}
}
__global__ void
parallel_softmax(float* in, float* out){
	...
	temp_sum = 1;
	int numTot = gridDim.x*blockDim.x;
	int myIdx = blockDim.x*blockIdx.x + threadIdx.x;
	
	temp[myIdx] = in[myIdx]*2;//w[myIdx*numTot+myIdy];
